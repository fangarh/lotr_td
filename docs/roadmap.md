...
7. Godot migration spike.
   ...
   - Next MVP implementation block: build spots and first tower purchasing. The accepted boundary is `gameState.startingGold`, `towers.catalog[].cost`, `towers.buildSpots`, a narrow `SpikeBuildStateAdapter`, `Main.build_tower_at_spot(...)`, and HUD intent controls, without mouse board picking, free placement, upgrades, selling/refunds, persistence, balance tuning, Android/touch-first UI, final build spot art, or path rerouting. The implementation plan is recorded at `docs/superpowers/plans/2026-05-04-godot-build-spots-purchasing.md`.
