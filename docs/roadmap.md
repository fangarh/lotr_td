...
7. Godot migration spike.
   ...
   - Build spots and first tower purchasing: done. Scenario data now includes `gameState.startingGold`, `towers.catalog[].cost`, and `towers.buildSpots`; `SpikeBuildStateAdapter` owns build/economy validation; `Main.build_tower_at_spot(...)` spawns purchased towers through the shared tower registration path; and `SpikeHud` exposes compact signal-only build controls with intermediate-width layout coverage. Done without mouse board picking, free placement, upgrades, selling/refunds, persistence, balance tuning, Android/touch-first UI, final build spot art, or path rerouting.
   - Next recommended MVP block: one-level tower upgrades for purchased/prebuilt towers, still using scenario data and a narrow rules boundary before any branch upgrades, selling/refunds, persistence, or balance pass.
