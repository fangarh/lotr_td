...
7. Godot migration spike.
   ...
   - Next recommended Godot direction: move from spike slices into the accepted two-map desktop MVP corridor. The first implementation block should establish scenario selection/loading for two maps and preserve data-driven scenario boundaries before adding build spots, tower purchasing, economy, one-level upgrades, four MVP enemy roles, and two balanced authored maps. Blocker behavior for MVP stays contact-stop plus HP/lifetime; path rerouting, stacking policy, final blocker visuals, Android/touch-first UX, free placement, upgrade branches, complex enemy abilities, and full campaign progression remain out of scope. The first implementation plan is recorded at `docs/superpowers/plans/2026-05-03-godot-scenario-selection-foundation.md`.
