...
const OUTPUT_PATH := "res://builds/previews/free_peoples_enemy_spike_preview.png"
...
func _capture() -> void:
	...
	if error != OK:
		push_error("Failed to save preview PNG: %s" % error_string(error))
		quit(1)
		return

	print("capture_free_peoples_enemy_preview: saved %s" % OUTPUT_PATH)
	quit(0)
