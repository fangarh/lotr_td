...
const OUTPUT_PATH := "res://builds/previews/spike_hud_preview.png"
...
func _capture() -> void:
	...
	if error != OK:
		push_error("Failed to save HUD preview PNG: %s" % error_string(error))
		quit(1)
		return

	print("capture_spike_hud_preview: saved %s" % OUTPUT_PATH)
	quit(0)
